...
var wind_node = AudioStreamPlayer.new()
...
func _ready():
	...
	wind_node.pause_mode = Node.PAUSE_MODE_PROCESS
	...
	stream = load("res://Assets/Sound_Effects/NightWinds.ogg")
	wind_node.set_stream(stream)
	...
	get_tree().get_root().call_deferred("add_child",wind_node)
	#music_node.play()
